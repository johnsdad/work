                  ...
                  <tbody>
                      ...
                      <?php if($input->input_type == 1) { ?>
                       <tr>
                          ...
                          <td><a href="manager/user_work/<?=$input->user_id?>"><?=$input->agent_name?></a></td>
                          <td><a href="manager/project_details/<?=$input->project_id?>"><?=$input->project_name?></a></td>
                          <td><?=$input->activity?></td>
                          <td><?= sprintf("%02d", $input->hours) ?> : <?= sprintf("%02d", $input->minutes) ?></td>
...
<script type="text/javascript">
  function input_view(input_id) {
    if(input_id) {
      $.ajax({
          url: 'manager/view_input/'+input_id,
          type: 'POST',
  ...
  function input_edit(input_id) {
    if(input_id) {
      $.ajax({
          url: 'manager/edit_input/'+input_id,
          type: 'POST',
          success: function (response) {
              $("#viewBody").html(response);
              $("#viewModal").modal('show');
      ...
      $.confirm({
          ...
          buttons: {
            unapprove: {
              ...
              action: function () {
                $.ajax({
                    url: 'manager/input_action/' + input_id  +'/' + valu,
                    type: 'POST',
                    success: function (response) {
