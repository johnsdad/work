...
class Manager extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        if (!$this->session->userdata('valid_session') || $this->session->userdata('valid_session')['type'] != 2) {
    ...
    public function index() {
        ...
        $data['users'] = $this->user->get_manager_users($this->session->userdata('valid_session')['user_id']);
        $data['projects'] = $this->project->get_unclosed_projects();
        $data['requests'] = $this->input1->get_manager_inputs($this->session->userdata('valid_session')['user_id']);
    ...
    public function add_user() {
        $this->load->view('manager/add_user');
    }
    ...
    public function show_user($id) {
        $data['user'] = $this->user->get_user($id);
        $this->load->view('manager/ajax/user_details', $data);
    }
    ...
    public function add_user_fields() {
        if(isset($_POST['type'])) {
            ...
            $this->load->view('manager/ajax/user_fields', $data);
        }
    }
    ...
    public function departments() {
        ...
        $data['departments'] = $this->department->get_departments();
        $this->load->view('manager/departments', $data);
    }

    public function edit_department($id) {
        $data['department'] = $this->department->get_department($id);
        $this->load->view('manager/ajax/edit_department', $data);
    }
    ...
    public function edit_project($id) {
        ...
        $data['departments'] = $this->department->get_departments();
        $this->load->view('manager/ajax/edit_project', $data);
    }

    public function show_project($id) {
        $data['project'] = $this->project->get_project($id);
        $this->load->view('manager/ajax/show_project', $data);        
    }
    ...
    public function closed_projects() {
        ...
        $this->load->view('manager/closed_projects', $data);
    }

    //////////////////////////////////////////////// Reports /////////////////////////////////////////////////

    public function pending_requests() {
        $data['inputs'] = $this->input1->get_manager_inputs($this->session->userdata('valid_session')['user_id']);
        $this->load->view('manager/pending_requests', $data);
    }
        ...
        if($this->input->post('input')){
            if(isset($_POST['id'])) {
                ...
                if($this->input1->update($this->input->post('id'), $data)) {
                    ...
                    redirect('manager/pending_requests');
                    exit;
                }
                $this->session->set_flashdata('error', 'Something went Wrong.');
                redirect('manager/pending_requests');
    ...
    public function project_work() {
        $data['projects'] = $this->project->get_unclosed_projects();
        $this->load->view('manager/project_work', $data);
    }

    public function closed_project_work() {
        $data['projects'] = $this->project->get_type_projects(2);
        $this->load->view('manager/closed_project_work', $data);
    }
    ...
    public function project_details($project_id) {
        ...
        $this->load->view('manager/project_details', $data);
    }

    public function user_work($user_id) {
        $data['user'] = $this->user->get_user($user_id);
        $data['user_id'] = $user_id;
        $this->load->view('manager/user_work', $data);
    }
